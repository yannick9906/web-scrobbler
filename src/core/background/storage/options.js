...
export const USE_UNRECOGNIZED_SONG_NOTIFICATIONS = 'useUnrecognizedSongNotifications';
...
export async function getConnectorOption(connector, key) {
...
export async function setConnectorOption(connector, key, value) {
	assertValidConnectorOptionKey(connector, key);
 ...
 */
export async function isConnectorEnabled(connector) {
	const data = await options.get();
 ...
 */
export async function setConnectorEnabled(connector, state) {
	const data = await options.get();
 ...
 * @param  {Boolean} state True if connector is enabled; false otherwise
 */
export async function setAllConnectorsEnabled(state) {
	const data = await options.get();
