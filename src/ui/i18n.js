export function translateDocument() {
	new PageTranslator().translateDocument();
}
...
class PageTranslator {
	...
	translateDocument() {
		...
		this.localizeElementChilds(document);
		...
		new MutationObserver((mutations) => {
			for (const mutation of mutations) {
				for (const node of mutation.addedNodes) {
					...
					this.localizeElementChilds(node);
				}
			}
	...
	localizeElement(element) {
		...
		for (const attr of I18N_ATTRS) {
			...
			switch (attr) {
				case 'i18n':
					if (PageTranslator.hasHtmlTags(text)) {
						const nodes = this.makeNodes(text);
						if (nodes) {
							nodes.forEach((n) => {
	 ...
	 */
	makeNodes(rawHtml) {
		let body = this.domParser.parseFromString(rawHtml, 'text/html').body;
